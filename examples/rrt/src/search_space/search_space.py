...
class SearchSpace(object):
    ...
    # 함수 발동 : def collision_free에서 호출, def sample_free에서 호출한다, rrt_base.py에 있는 new_and_near에서 호출(새로 생성한 내분점이 장애물에 위치하는가 체크하기 위해서)
    # 입력 변수
    # 함수 내용 
    # self.obs.count(x) 위치가 장애물 내에 있는지 확인하는 함수 , x = 확인할 위치
    ...
    def collision_free(self, start, end, r):
        ...
        # map으로 데이터를 묶어준다음, all 내장함수를 사용해서 bool형태로 반환받는다.
